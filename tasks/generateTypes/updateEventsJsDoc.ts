...
export function updateEventsJsDoc() {
  ...
  // @ts-ignore
  const allAttributes: IAttributeData[] = htmlDataImport.globalAttributes.concat(...htmlDataImport.tags.map(x => x.attributes as IAttributeData[]));

  // Used to not repeat comments - added some deprecated events
  const processedAttributes = new Set<string>(["onformchange", "onforminput", "onmousewheel", 
  ...
  for (const attr of allAttributes) {
    ...
    if (!eventName.startsWith('on') || attr.status?.baseline === false || processedAttributes.has(eventName))
      continue;
    const jsDoc = getJSDoc(attr);
    if (!jsDoc)
      continue;
    if(!content.includes(eventName))
      console.log(`Missing event ${eventName}`)
