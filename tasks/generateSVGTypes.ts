...
const getPropertyValue = (property: Attribute) => {
  ...
  if (property.values) {
    const values = property.values.map((x) => x.name);
    const valueSetIndexFound = generatedSvgValueSets.findIndex((x) =>
      values.every((y) => x?.includes(y)),
    );
    if (valueSetIndexFound === -1) {
      const newValueSetIndex = generatedSvgValueSets.push(values) - 1;
...
const svgElements = svgData.tags
  ?.filter((x) => !NotSupportedSVGTags.includes(x.name))
...
${getPropertyName(
  currentValue.name,
      ...
      currentValue,
    )} & SVGEvents<SVGElementTagNameMap['${currentValue.name}']>;`;
  }, "");

const SVGValueSets = generatedSvgValueSets?.reduce(
  (previousValue, currentValue, i) => {
    return `${previousValue}
  [${i}]: ${currentValue.map((x) => `"${x}"`).join(" | ")};`;
  },
...
mkdirSync("./src/generated/SVGElements", {
  recursive: true,
});
